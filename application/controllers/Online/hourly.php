<?php
class Hourly extends MY_Controller
{
	public function __construct()
	{
		log_message('debug', '=====开始调用Online->Hourly->__construct接口=====');
		parent::__construct();
		$this->loadLanguageFile('application/controllers/Online/');
		log_message('debug', '=====调用Online->Hourly->__construct接口结束=====');
	}
	
	public function showView() {
		$this->load->view('module/Online/hourly.htm');
	}
	
	public function getListData() {
		log_message('debug', '=====开始调用Online->Hourly->getListData接口=====');
		...
		try {
			...
			$getDataSql = str_replace('<selectColumns>', "FROM_UNIXTIME(a.`time`) as `time`, a.`regist_new`, a.`enter_new`, round(a.`enter_new`/a.`regist_new`*100,2) as `rate`,a.`login_count`,a.`login_player`,a.`online_max`,a.`online_min`,a.`online_avg`,a.`pay_count`,a.`pay_player`,round(a.`pay_count`/a.`pay_player`,2) as `ARPU`,round(a.`pay_player`/a.`enter_new`*100,2) as `pay_rate`", $sql);
		...
		$this->returnJson();
		log_message('debug', '=====调用Online->Hourly->getListData接口结束=====');
	}
        public function export() {
		log_message('debug', '=====开始调用Online->Hourly->export接口=====');
		$this->verifyPrivilege();
		...
		log_message('debug', '=====调用Online->Hourly->export接口结束=====');
	}
}