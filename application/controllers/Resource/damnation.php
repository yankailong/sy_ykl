<?php
class Damnation extends MY_Controller
{
	public function __construct()
	{
		log_message('debug', '=====开始调用Resource->Damnation->__construct接口=====');
		parent::__construct();
		$this->loadLanguageFile('application/controllers/Resource/');
		log_message('debug', '=====调用Resource->Damnation->__construct接口结束=====');
	}
	
	public function showView() {
		$this->load->view('module/Resource/damnation.htm');
	}
	
	public function getListData() {
		log_message('debug', '=====开始调用Resource->Damnation->getListData接口=====');
		...
		try {
			...
			$this->returnData['data']['total'] = $this->gmDatabase->getColumn($getTotalSql);
                        $getDataSql =str_replace('<selectColumns>', "from_unixtime(a.`time`,'%y-%m-%d') as `date`,((a.`time`-unix_timestamp(date_format(b.`openDatetime`,'%Y-%m-%d')))/86400)+1 as `days`,a.`damnation` as `ave`", $sql);
		...
		$this->returnJson();
		log_message('debug', '=====调用Resource->Damnation->getListData接口结束=====');
	}
        public function export() {
		log_message('debug', '=====开始调用Resource->Damnation->export接口=====');
		$this->verifyPrivilege();
		@ini_set('memory_limit','1024M');
		try {
			$fileName = "damnation.txt";
			...
			if($endDate) {
				...
				";
                        ...
                        $getDataSql =str_replace('<selectColumns>', "from_unixtime(a.`time`,'%y-%m-%d') as `date`,((a.`time`-unix_timestamp(date_format(b.`openDatetime`,'%Y-%m-%d')))/86400)+1 as `days`,a.`damnation`", $sql);
			...
			$this->returnData['data']['rows'] = $this->gmDatabase->getArray($getDataSql);
                        ...
                        foreach ($this->returnData['data']['rows'] as $key => $value) {
                            echo "{$value['date']}\t{$value['days']}\t{$value['damnation']}\t\n";
                        }
		}
		...
		log_message('debug', '=====调用Resource->Damnation->export接口结束=====');
	}
        public function player() {
		log_message('debug', '=====开始调用Resource->Damnation->player接口=====');
		$this->verifyPrivilege();
		...
		try {
			...
			";
                        ...
                        $getTotalSql = str_replace('<selectColumns>', "count(*)", $sql);
			$this->returnData['data']['total'] = $this->gmDatabase->getColumn($getTotalSql);
                        $getDataSql =str_replace('<selectColumns>', "((a.`time`-unix_timestamp(date_format(b.`openDatetime`,'%Y-%m-%d')))/86400)+1 as `days`,a.`cid2` as `cid`,a.`name2` as `name`,a.`damnation`,a.`order`", $sql);
		...
		$this->returnJson();
		log_message('debug', '=====调用Resource->Damnation->player接口结束=====');
	}
}