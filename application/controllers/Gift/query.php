...
class Query extends MY_Controller
{
	...
	{
		log_message('debug', '=====开始调用Gift->Query->__construct接口=====');
		parent::__construct();
		$this->loadLanguageFile('application/controllers/Gift/');
		log_message('debug', '=====调用Gift->Query->__construct接口结束=====');
	}
	
	public function showView() {
		$this->load->view('module/Gift/query.htm');
	}
	
	public function getListData() {
		log_message('debug', '=====开始调用Gift->Query->getListData接口=====');
		...
		try{
			...
			$cardName = $this->param->getParam('cardName', array('method' => 'string', 'emptyErrorCode' => 1, 'invalidErrorCode' => 1), 'post', true);
			...
			";
			if($cardName) {
				$sql .= " and a.`cardName` = '{$cardName}'";
			}
			if($startDateTime) {
				$sql .= " and unix_timestamp(b.`applyDatetime`) >= unix_timestamp('{$startDateTime} 00:00:00')";
			}
			
			if($endDateTime) {
				$sql .= " and unix_timestamp(b.`applyDatetime`) <= unix_timestamp('{$endDateTime} 23:59:59')";
			}
			...
			$getDataSql .= " order by b.`applyDatetime` desc";
	...
	public function create() {
		log_message('debug', '=====开始调用Gift->Query->create接口=====');
		$this->verifyPrivilege();
		...
		$this->returnJson();
		log_message('debug', '=====调用Gift->Query->create接口结束=====');
	}
        
    public function export() {
		log_message('debug', '=====开始调用Gift->Query->export接口=====');
		$this->verifyPrivilege();
    ...
    public function del() {
		log_message('debug', '=====开始调用Gift->Query->del接口=====');
		$this->verifyPrivilege();
