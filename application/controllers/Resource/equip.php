...
class Equip extends MY_Controller
{
	...
	{
		log_message('debug', '=====开始调用Resource->Equip->__construct接口=====');
		parent::__construct();
		$this->loadLanguageFile('application/controllers/Resource/');
		log_message('debug', '=====调用Resource->Equip->__construct接口结束=====');
	}
	
	public function showView() {
		$this->load->view('module/Resource/equip.htm');
	}
	
	public function getListData() {
		log_message('debug', '=====开始调用Resource->Equip->getListData接口=====');
		...
		try {
			...
			$this->returnData['data']['total'] = $this->gmDatabase->getColumn($getTotalSql);
                        $getDataSql =str_replace('<selectColumns>', "from_unixtime(a.`time`,'%y-%m-%d') as `date`,((a.`time`-unix_timestamp(date_format(b.`openDatetime`,'%Y-%m-%d')))/86400)+1 as `days`,a.`equip` as `ave`", $sql);
		...
		$this->returnJson();
		log_message('debug', '=====调用Resource->Equip->getListData接口结束=====');
	}
        public function export() {
		log_message('debug', '=====开始调用Resource->Equip->export接口=====');
		$this->verifyPrivilege();
		...
		try {
			...
			if($endDate) {
				...
				";
                        ...
                        $getDataSql =str_replace('<selectColumns>', "from_unixtime(a.`time`,'%y-%m-%d') as `date`,((a.`time`-unix_timestamp(date_format(b.`openDatetime`,'%Y-%m-%d')))/86400)+1 as `days`,a.`equip`", $sql);
			...
			$this->returnData['data']['rows'] = $this->gmDatabase->getArray($getDataSql);
                        ...
                        foreach ($this->returnData['data']['rows'] as $key => $value) {
                            echo "{$value['date']}\t{$value['days']}\t{$value['equip']}\t\n";
                        }
		}
		...
		log_message('debug', '=====调用Resource->Equip->export接口结束=====');
	}
        public function player() {
		log_message('debug', '=====开始调用Resource->Equip->player接口=====');
		$this->verifyPrivilege();
		...
		try {
			...
			";
                        ...
                        $getTotalSql = str_replace('<selectColumns>', "count(*)", $sql);
			$this->returnData['data']['total'] = $this->gmDatabase->getColumn($getTotalSql);
                        $getDataSql =str_replace('<selectColumns>', "((a.`time`-unix_timestamp(date_format(b.`openDatetime`,'%Y-%m-%d')))/86400)+1 as `days`,a.`cid1` as `cid`,a.`name1` as `name`,a.`equip`,a.`order`", $sql);
		...
		$this->returnJson();
		log_message('debug', '=====调用Resource->Equip->player接口结束=====');
	}
}